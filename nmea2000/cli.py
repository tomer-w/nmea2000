...
def _add_common_client_args(sub: argparse.ArgumentParser):
    ...
    sub.add_argument("--dump_file", type=str, help="Record frames to a given file")
    sub.add_argument(
        "--dump_pgns", type=str, help="Record only specific PGNs, comma separated"
    )
    sub.add_argument(
        "--json",
        action="store_true",
...
async def async_main():
    ...
    decode_parser.add_argument(
        "--file", type=str, help="Path to a file containing supported NMEA 2000 frames"
    )
    decode_parser.add_argument(
        "--single_line",
        action="store_true",
    ...
    encode_parser.add_argument(
        "--frame", type=str, help="json NMEA 2000 frame (optional if file is provided)"
    )
    encode_parser.add_argument(
        "--file", type=str, help="Path to a file containing a JSON NMEA 2000 frame"
    )
    ...
    ebyte_parser = subparsers.add_parser(
        "ebyte", help="Connect to an EByte ECAN-E01/W01 TCP gateway"
    )
    ...
    waveshare_parser = subparsers.add_parser(
        "waveshare", help="Connect to a WaveShare USB-CAN-A serial gateway"
    )
    ...
    if args.command == "decode":
        ...
        logger.info(
            "Using EByteNmea2000Gateway with server: %s, port: %d",
            args.server,
            args.port,
        )
        ...
        logger.info(
            "Using TextNmea2000Gateway (%s) with server: %s, port: %d",
            fmt_label,
            args.server,
            args.port,
        )
        ...
        logger.info(
            "Using ActisenseBstNmea2000Gateway with server: %s, port: %d",
            args.server,
            args.port,
        )
        ...
        client = PythonCanAsyncIOClient(
            dump_to_file=args.dump_file, dump_pgns=args.dump_pgns, **kwargs
        )
        await interactive_client(client, json_output=args.json)
